...
void StageManager::onTouchesEnded(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event  *event)
{
	...
	if (!touches.empty())
	{
		...
		Vec3 dir(farP - nearP);
		CCLOG("nearP -> %g %g %g", nearP.x, nearP.y, nearP.z);
		CCLOG("farP -> %g %g %g", farP.x, farP.y, farP.z);
		CCLOG("%g %g", location.x, location.y);
...
void StageManager::onTouchesMoved(const std::vector<cocos2d::Touch*>& touches, cocos2d::Event  *event)
{
	if (!touches.empty())
	{
		...
		_camera->setPosition3D(Vec3(200.0f * sinf(_angle), 200.0f * cosf(_angley), 200.0f * (cosf(_angle)+sinf(_angley))));
